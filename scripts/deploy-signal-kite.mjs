...
const source = await readFile(resolve(process.cwd(), "contracts/SignalKite.sol"), "utf8");
const input = {
  language: "Solidity",
  sources: { "SignalKite.sol": { content: source } },
  settings: { outputSelection: { "*": { "*": ["abi", "evm.bytecode.object"] } } },
};
const output = JSON.parse(solc.compile(JSON.stringify(input)));
const contract = output.contracts?.["SignalKite.sol"]?.SignalKite;
if (!contract) {
...
console.log(`Deploying SignalKite from ${account.address} on Base...`);
...
console.log("Set NEXT_PUBLIC_SIGNAL_KITE_CONTRACT_ADDRESS to this contract address in Vercel.");
